	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (xiaomi_msm8937_mach_get_family() == XIAOMI_MSM8937_MACH_FAMILY_ULYSSE) {
		...
		ctrl_pdata->xiaomi_ulysse_board_id0_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node, "qcom,xiaomi-ulysse-board-id-gpios", 0);
		if(!gpio_is_valid(ctrl_pdata->xiaomi_ulysse_board_id0_gpio))
			pr_info("%s: xiaomi_ulysse_board_id0_gpio not specified\n",__func__);

		ctrl_pdata->xiaomi_ulysse_board_id1_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node, "qcom,xiaomi-ulysse-board-id-gpios", 1);
